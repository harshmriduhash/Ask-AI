  ...
  return (
    <>
      <div className="flex min-h-screen justify-center bg-slate-900 p-4">
        <div className="max-w-2xl rounded bg-zinc-900 p-4 text-red-200 shadow-md">
          {/* Render the streaming data here */}
          {streamData.map((data, index) => (
            <div key={index}>{data}</div>
          ))}
        </div>
        <div className="fixed bottom-1 left-1/2 m-8 flex min-h-12 w-1/4 -translate-x-1/2 items-center justify-between overflow-hidden rounded-lg bg-slate-300 px-0">
          <input
            type="text"
